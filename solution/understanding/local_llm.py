"""
BidDesk LLM Interface
Authored by rigur_calypso

LLM interface for the BidDesk pipeline.
"""
...
def get_next_key():
    global _API_KEYS, _current_key_idx, _last_switch_time
    if not _API_KEYS:
        # Load from .env if available, or just from environment
        keys_str = os.environ.get('GEMINI_API_KEY', '')
        _API_KEYS = [k.strip() for k in keys_str.split(',') if k.strip()]
        if not _API_KEYS:
            raise ValueError("No API keys found in GEMINI_API_KEY")
    
    # If we recently switched, don't switch again immediately (prevent cascade)
    if time.time() - _last_switch_time > 2:
        _current_key_idx = (_current_key_idx + 1) % len(_API_KEYS)
        _last_switch_time = time.time()
        print(f"  [LLM Engine] Switched to API key index {_current_key_idx}.")
...
def call_gemini(prompt: str, model_name: str = 'gemini-3.5-flash', retries: int = 50) -> str:
    global _current_key_idx, _api_usage_count
    if not _API_KEYS:
        raise ValueError("No API keys found in GEMINI_API_KEY")
            ...
            if "429" in err_str or "quota" in err_str.lower():
                with _key_lock:
                    if _current_key_idx == idx:
                        _current_key_idx = (_current_key_idx + 1) % len(_API_KEYS)
                        print(f"  [LLM Engine] Rate limit on key {idx}. Switched to {_current_key_idx}.")
                        if _current_key_idx == 0:
                            print(f"  [LLM Engine] Sleeping 20s to allow quotas to reset...")
                            time.sleep(20)
                continue
            else:
                raise e
    raise RuntimeError("All keys exhausted and failed.")
