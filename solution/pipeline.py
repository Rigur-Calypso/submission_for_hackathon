#!/usr/bin/env python3
"""
BidDesk Core Pipeline Engine
Authored by rigur_calypso

This module orchestrates the processing of incoming queries, routing them to the appropriate subsystem (DB or RAG fallback).
...
logger = logging.getLogger("biddesk.pipeline")
...
def run(db_path: str, questions_path: str, out_path: str,
    ...
    def process_item(item, idx, total):
        # Local connection per thread because sqlite3 connections aren't strictly thread-safe in some setups
        local_con = get_connection(db_path)
        qid = item["qid"]
        question = item["question"]
        answer_type = item.get("answer_type", "money")
        logger.info(f"[{idx}/{total}] {qid}: processing...")
        try:
            answer, meta = answer_question(local_con, question, answer_type=answer_type, log=log)
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        futures = {executor.submit(process_item, item, idx, total): item for idx, item in enumerate(questions, 1)}
        for future in as_completed(futures):
            results.append(future.result())

    # Sort results to match original order if needed, but not strictly necessary for CSV
    # Usually CSV evaluation doesn't care about order, but let's be safe
    # Actually just write them out
