...
- **Isolated Inputs**: All input files (e.g. `questions.json`, `document_index.csv`, `documents/` folder, `extracted_text/` folder, etc.) are kept in the parent/root directory of the project. The `solution` folder does not contain duplicate copies of these files.
- **Dedicated Outputs (`solution/output/`)**: All outputs generated by the pipeline execution are saved here:
...
- **Root-level answers**: A copy of `submission.csv` is also saved as `solution/submission.csv`.
...
5. Converts the JSONL output into `output/submission.csv` and copies it to `solution/submission.csv`.
...
```bash
python solution/run.py --questions ../questions.json --delay 1.0
```
